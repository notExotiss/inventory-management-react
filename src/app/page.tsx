...
function InventoryApp() {
  ...
  const locationOptions = useMemo(() => {
    ...
    const traverse = (containerList: Container[]) => {
      containerList.forEach((container) => {
        if (container.containerLocation?.path) {
          paths.add(container.containerLocation.path);
        }
  ...
  const triggerItemShake = (itemId: string) => {
  ...
  const handleDragEnd = (event: any) => {
    ...
    if (activeData.type === "item" && overData.type === "container") {
      ...
      if (!sourceContainerId) {
        setShowFolderWarning(true); // Open the popup
        triggerItemShake(itemId);
        return;
      }
    ...
    if (activeData.type === "container" && overData.type === "container") {
      ...
      if (
        ...
        const sourceContainer =
          activeData.container || findContainerById(containerId);

        const targetContainer =
          overData.container || findContainerById(targetContainerId);
  ...
  return (
    <DndProviderWrapper key={dndKey} onDragEnd={handleDragEnd}>
      <div className="app-container">
              ...
              {showAllItemsView ? (
                <MobileAllItemsView
                  ...
                  onMoveItem={(itemId, targetContainerId) => {
                    ...
                    const sourceContainer = findItemContainer(
                      containers,
                      itemId,
                    );
                    if (sourceContainer) {
                ...
                <div className="p-6">
                  <div className="mb-6">
                    <h2 className="text-2xl font-semibold mb-1">
                      {selectedContainer
                        ? findContainerById(selectedContainer)?.containerName ||
                          "Location"
                    ...
                    {selectedContainer && (
                      <p className="text-sm text-muted-foreground">
                        {
                          findContainerById(selectedContainer)
                            ?.containerLocation.path
                        }
                      </p>
                    )}
                  </div>
        ...
        <Dialog open={showFolderWarning} onOpenChange={setShowFolderWarning}>
          <DialogContent className="sm:max-w-[425px]">
            <DialogHeader>
              <div className="flex items-center gap-2 text-amber-500">
                <AlertCircle className="h-5 w-5" />
                <DialogTitle>Cannot Move Item</DialogTitle>
              </div>
              <DialogDescription className="pt-2">
                PLEASE NAVIGATE TO A FOLDER BEFORE DRAGGING. You cannot move
                items directly from the All Items view without a source folder.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
              <button
